...
type Props = {
  ...
  className?: string
  title?: string
  /** when true, width is fluid at all breakpoints; when false, md+ gets a fixed width */
  fixedWidth?: boolean

  /** --- Back-compat props (pages still use these) --- */
  teamId?: string
...
export default function TeamPill(props: Props) {
  const {
    ...
    fluid,
    labelMode = 'auto',
    // variant not used visually for now, but kept to avoid type errors
  ...
  // width logic: legacy `fluid` overrides new `fixedWidth`
