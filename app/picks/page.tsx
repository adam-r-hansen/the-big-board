...
function TeamButton({ team, disabled, picked, onClick }: { team?: TeamLike; disabled?: boolean; picked?: boolean; onClick?: () => void }) {
  ...
  return (
    <button
      ...
      className={['w-full h-14 rounded-xl border px-4 font-semibold tracking-wide','transition-[transform,opacity] active:scale-[0.98]', disabled ? 'opacity-40 cursor-not-allowed' : 'hover:opacity-90'].join(' ')}
      style={{ borderColor: primary, color: primary, boxShadow: picked ? `inset 0 0 0 2px ${primary}` : undefined, background: picked ? `linear-gradient(0deg, ${secondary}22, ${secondary}10)` : 'transparent' }}
    >{abbr}</button>
  )
}
...
function SectionCard({ children, title, right }: { children: React.ReactNode; title: string; right?: React.ReactNode }) {
  return (
    <section className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4 md:p-5">
      <header className="mb-3 flex items-center justify-between"><h2 className="text-lg font-semibold">{title}</h2>{right}</header>
      {children}
    </section>
  )
}
...
export default function PicksPage() {
  ...
  useEffect(() => {
    fetch('/api/my-leagues').then(r=>r.json()).then(j=>{ const ls: League[] = j.leagues || []; setLeagues(ls); if (!leagueId && ls[0]) { setLeagueId(ls[0].id); setSeason(ls[0].season) } })
    fetch('/api/team-map').then(r=>r.json()).then(j=> setTeamMap(j.teams || {}))
  ...
  const teamIndex: Record<string, TeamLike> = useMemo(() => {
    ...
    for (const t of Object.values(teamMap)) {
      const v: TeamLike = { id: t.id, abbreviation: t.abbreviation ?? undefined, name: t.name ?? undefined, color_primary: t.color_primary ?? undefined, color_secondary: t.color_secondary ?? undefined, logo: t.logo ?? undefined, logo_dark: t.logo_dark ?? undefined }
      if (t.id) idx[t.id] = v
    ...
    ;(async () => {
      ...
      try {
        ...
        setGames((g.games ?? []).map((x: any) => ({
          id: x.id, game_utc: x.game_utc || x.start_time, status: x.status ?? 'UPCOMING', season: x.season, week: x.week,
          home: { id: x.home?.id ?? x.home_team ?? x.homeTeamId ?? x.home_team_id, abbr: x.home?.abbreviation ?? x.home_abbr ?? x.homeAbbr ?? x.home?.abbr ?? null },
          away: { id: x.away?.id ?? x.away_team ?? x.awayTeamId ?? x.away_team_id, abbr: x.away?.abbreviation ?? x.away_abbr ?? x.awayAbbr ?? x.away?.abbr ?? null },
        })))
        setPicks((p.picks ?? []).map((r: any) => ({ id: r.id, team_id: r.team_id, game_id: r.game_id })))
      } catch (e: any) { setMsg(e?.message || 'Load error') } finally { setLoading(false) }
    })()
  ...
  async function safeJson(res: Response) { const ct = res.headers.get('content-type') || ''; if (ct.includes('application/json')) { try { return await res.json() } catch { return null } } return null }
  ...
  async function togglePick(teamId: string, gameId: string) {
    try {
      ...
      const res = await fetch('/api/picks', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ leagueId, season, week, teamId, gameId }), cache: 'no-store' })
      if (!res.ok) { const j = await res.json().catch(()=> ({})); throw new Error(j?.error || `HTTP ${res.status}`) }
      await refreshMyPicks(); setMsg('')
    } catch (e: any) { const m = e?.message || 'Pick error'; setMsg(m); console.error('togglePick error', m) }
  }
                ...
                {picks.map((p) => {
                  ...
                  return (
                    <li key={p.id} className="flex items-center justify-between rounded-lg border border-neutral-200 dark:border-neutral-800 px-3 py-2">
                      ...
                      <button className="text-xs underline disabled:opacity-50" disabled={locked} onClick={async () => { try { await deletePickById(p.id); await refreshMyPicks(); setMsg('') } catch (e: any) { const m = e?.message || 'Unpick failed'; setMsg(m); console.error('unpick error', m) } }} title={locked ? 'Locked (kickoff passed)' : 'Unpick'}>{locked ? 'Locked' : 'Unpick'}</button>
                    </li>
                  )
                })}
              </ul>
