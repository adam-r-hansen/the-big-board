...
export default function LeagueWrinklesPage() {
  ...
  async function onCreate() {
    ...
    try {
      ...
      // 2) Attach game(s) based on type
      if (isOOF) {
        // Auto-hydrate OOF wrinkles
        const oofRes = await fetch(`/api/admin/wrinkles/${wrinkleId}/hydrate-oof`, {
          method: 'POST',
          credentials: 'same-origin',
          headers: { 'content-type': 'application/json' },
        })
        const { data: oofData, text: oofText } = await readJsonSafe(oofRes)
        if (!oofRes.ok) {
          throw new Error(oofData?.error || `OOF hydrate failed (${oofRes.status}) ${oofText?.slice(0,200)}`)
        }
        setLog(`✅ Created with ${oofData?.qualifying_games || 0} qualifying games (using Week ${oofData?.records_week || week - 1} records)`)
      } else if (needsGame && gameId) {
        // Manual game selection for bonus_game and bonus_game_ats
        ...
        {isOOF && (
          <div className="rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900 p-3">
            <p className="text-sm text-amber-900 dark:text-amber-200">
              <strong>OOF Bonus Pick:</strong> Games will be automatically selected based on teams with win percentage below .400 using Week {week - 1} records.
            </p>
          </div>
        )}
