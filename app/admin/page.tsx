...
type WrinkleInput = {
  ...
  extra_picks?: number
  week?: number
  kind?: 'bonus_game' | 'winless_double' | 'extra_picks_only'
}
...
export default function AdminPage() {
  ...
  const [wrinkle, setWrinkle] = useState<WrinkleInput>({
    name: '',
    description: '',
    extra_picks: 0,
    week: 1,
  ...
  async function createWrinkle() {
    if (!leagueId || !wrinkle.name || !wrinkle.kind) return
    setBusy('wrinkle')
    try {
      await post('/api/admin/wrinkles', {
        ...
        status: 'active',
        kind: wrinkle.kind,
        extraPicks: wrinkle.extra_picks ?? 0,
        config: {}, // stored as config; API normalizes
      })
      setWrinkle({ name: '', description: '', extra_picks: 0, week: 1, kind: 'bonus_game' })
            ...
            <div className="lg:col-span-8 grid gap-6">
              ...
              <Card title="Wrinkle Creator (League)">
                <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
                  ...
                  <select className="border rounded px-2 py-2 bg-transparent md:col-span-1"
                          value={wrinkle.kind} onChange={e => setWrinkle(v => ({ ...v, kind: e.target.value as any }))}>
                    <option value="bonus_game">bonus_game</option>
                    <option value="winless_double">winless_double</option>
                    <option value="extra_picks_only">extra_picks_only</option>
                  </select>
                  <input className="border rounded px-3 py-2 bg-transparent md:col-span-1" placeholder="Extra picks (0)" type="number"
                        value={wrinkle.extra_picks ?? 0} onChange={e => setWrinkle(v => ({ ...v, extra_picks: Number(e.target.value) }))}/>
                  <input className="border rounded px-3 py-2 bg-transparent md:col-span-1" placeholder="Week" type="number"
                        value={wrinkle.week ?? 1} onChange={e => setWrinkle(v => ({ ...v, week: Number(e.target.value) }))}/>
                  <Button className="md:col-span-1" disabled={!leagueId || !wrinkle.name || busy==='wrinkle'} onClick={createWrinkle}>
                      ...
                      <tbody>
                        {seasonWrinkles.map((w) => (
                          <tr key={w.id} className="border-t border-neutral-200 dark:border-neutral-800 align-top">
                            ...
                            <td className="py-3">
                              {w.kind !== 'bonus_game' ? (
                                <span className="text-xs text-neutral-500">No game attachment for this kind.</span>
                              ) : (
